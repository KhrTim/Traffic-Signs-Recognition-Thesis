...
class FileInputScreen(ctk.CTkToplevel):
    def __init__(self, parent, model_type: ModelTypes):
        ...
        load_asset = Image.open(
            "./assets/search_FILL0_wght400_GRAD0_opsz48.png")
        ...
        image_frame = ctk.CTkFrame(
            self, width=1200, height=800, fg_color='transparent')
        self.img_widget = ctk.CTkLabel(image_frame, text=None)
        if self.source_controller and self.source_controller.current_mode() == SourceMode.VIDEO:
    ...
    def change_source_controller(self, filename):
        ...
        if not self.source_controller or self.source_controller.current_filename() != filename:
            if self.source_controller:
                self.source_controller.clear()
